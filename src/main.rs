    ...
    pub fn update(&mut self, message: Message) {
        match message {
            ...
            Message::AddModule(module) => {
                ...
                let variables = waves.inner.variables_in_scope(&module);
            ...
            Message::WavesLoaded(filename, format, new_waves, load_options) => {
                ...
                let new_wave = if load_options.keep_variables && self.waves.is_some() {
                    ...
                    WaveData {
                        ...
                        cursors: HashMap::new(),
                        focused_item: None,
                        default_variable_name_type: self.config.default_signal_name_type,
                        scroll_offset: 0.,
                        item_offsets: vec![],
            ...
            Message::ToggleVariableTooltip => {
                let new = match self.show_variable_tooltip {
                    Some(prev) => !prev,
                    None => !self.config.layout.show_variable_tooltip(),
                };
