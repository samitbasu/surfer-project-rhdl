        ...
        TimeStringFormatting::SI => {
            if time.contains('.') {
                ...
                let integer_result = if integer_part.len() > 4 {
                    group_n_chars(integer_part, 3).join("\u{202F}")
                } else {
                    integer_part.to_string()
                };
                if fractional_part.len() > 4 {
                    let reversed = fractional_part.chars().rev().collect::<String>();
                    let reversed_fractional_parts = group_n_chars(&reversed, 3).join("\u{202F}");
                ...
                if time.len() > 4 {
                    group_n_chars(&time, 3).join("\u{202F}")
                } else {
                    time
                }
            }
...
mod test {
    ...
    fn print_time_si() {
        assert_eq!(
            ...
            "123\u{202f}456.789\u{202f}01 s"
        );
        assert_eq!(
            time_string(
