...
#[inline]
fn shortest<T: std::fmt::LowerExp + std::fmt::Display>(v: T) -> String {
    let dec = format!("{v}");
    let exp = format!("{v:e}");
    if dec.len() > exp.len() {
        exp
...
impl NumericTranslator for SinglePrecisionTranslator {
    ...
    fn translate_biguint(&self, _: u64, v: num::BigUint) -> String {
        shortest(f32::from_bits(v.iter_u32_digits().next().unwrap_or(0)))
    }
...
impl NumericTranslator for DoublePrecisionTranslator {
    ...
    fn translate_biguint(&self, _: u64, v: num::BigUint) -> String {
        shortest(f64::from_bits(v.iter_u64_digits().next().unwrap_or(0)))
    }
    fn translates(&self, signal: &SignalMeta) -> Result<TranslationPreference> {
...
fn decode_e4m3(v: u8) -> String {
    ...
    match (exp, mant) {
        ...
        (0, ..) => shortest(((sign * mant as i8) as f32) * 0.001953125f32), // 0.001953125 = 2^-9
        _ => shortest(
