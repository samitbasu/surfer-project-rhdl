...
impl WaveData {
    ...
    pub fn draw_cursor_number_boxes(
        ...
        let text_size = ctx.cfg.text_size;
        let y_offset = (ctx.cfg.line_height - text_size) * 0.5;
        ...
        for displayed_item in self.displayed_items.iter().filter_map(|item| match item {
            ...
            let max = (ctx.to_screen)(rect.max.x, ctx.cfg.line_height);
            let min = Pos2::new(rect.min.x - y_offset, min.y);
            let max = Pos2::new(rect.max.x + y_offset, max.y);
