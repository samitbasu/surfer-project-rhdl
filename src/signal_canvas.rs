...
use crate::time::{draw_tick_line, time_string};
...
impl State {
    ...
    pub fn draw_signals(
        ...
        if let Some(draw_data) = &*self.sys.draw_data.borrow() {
                    ...
                    .unwrap_or_else(|| self.config.layout.show_ticks())
            {
                ...
                for (_, x) in ticks {
                    draw_tick_line(*x, &mut ctx, &stroke)
                }
            }
    ...
    fn get_ticks(&self, waves: &WaveData, frame_width: f32, text_size: f32) -> Vec<(String, f32)> {
        ...
        let max_labels = ((frame_width * self.config.ticks.density) / max_labelwidth).floor();
