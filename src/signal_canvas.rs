    ...
    pub fn draw_signals(&mut self, msgs: &mut Vec<Message>, ui: &mut egui::Ui) {
        ...
        response.dragged_by(egui::PointerButton::Primary).then(|| {
            ...
            if let Some(utimestamp) = timestamp.to_biguint() {
                let closest_item = &waves.displayed_items[waves.get_item_at_y(pos.y)];
                if let DisplayedItem::Signal(signal) = closest_item {
                    let res = waves.inner.query_signal(&signal.signal_ref, &utimestamp);
                    if res.is_ok() {
                        let rres = res.ok().unwrap();
                        let prev_time = &rres.current.unwrap().0.to_bigint();
                        let next_time = &rres.next.unwrap_or_default().to_bigint();
