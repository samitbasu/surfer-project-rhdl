                        ...
                        match gesture_type(start_location, end_location) {
                            ...
                            Some(GestureKind::ZoomIn) => {
                                ...
                                msgs.push(Message::ZoomToRange {
                                    start: waves.viewport.to_f64_time(minx as f64, frame_width),
                                    end: waves.viewport.to_f64_time(maxx as f64, frame_width),
                                })
                            }
    ...
    fn draw_zoom_in_gesture(
        ...
        draw_gesture_text(
            ...
            format!(
                "Zoom in: {} to {}",
                time_string(
                    &(waves.viewport.to_bigint_time(minx as f64, width)),
                    &waves.inner.metadata().timescale,
                    &(self.wanted_timeunit)
                ),
                time_string(
                    &(waves.viewport.to_bigint_time(maxx as f64, width)),
                    &waves.inner.metadata().timescale,
                    &(self.wanted_timeunit)
                ),
            ),
