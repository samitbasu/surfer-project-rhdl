...
impl State {
            ...
            ui.with_layout(Layout::top_down(alignment).with_cross_justify(true), |ui| {
                match displayed_item {
                    DisplayedItem::Signal(displayed_signal) => {
                        ...
                        if self.sys.command_prompt.visible
                            && expand_command(
                                &self.sys.command_prompt_text.borrow(),
                                get_parser(self),
                            )
                            .expanded
                            .starts_with("signal_focus")
                        {
                            self.add_alpha_id(vidx, &style, &mut layout_job);
                        }

                        self.add_focus_marker(vidx, &style, &mut layout_job);
    ...
    fn add_focus_marker(&self, vidx: usize, style: &Style, mut layout_job: &mut LayoutJob) {
    ...
    fn draw_plain_var(
        ...
        let mut draw_label = |ui: &mut egui::Ui| {
            ui.horizontal_top(|ui| {
                ...
                let signal_label = ui
                    .selectable_label(false, WidgetText::LayoutJob(layout_job))
                    .context_menu(|ui| {
    ...
    fn add_alpha_id(&self, vidx: usize, style: &Style, mut layout_job: &mut LayoutJob) {
