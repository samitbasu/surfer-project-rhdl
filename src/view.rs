...
impl State {
            ...
            ui.with_layout(
                ...
                |ui| {
                    ui.add(egui::SelectableLabel::new(false, sig.name.clone()))
                        .on_hover_text(self.signal_tooltip_text(wave, &sig))
                        .clicked()
    ...
    fn signal_tooltip_text(&self, wave: &WaveData, sig: &SignalRef) -> String {
        format!(
            "{}\nNum bits: {}\nType: {}",
        ...
        let mut draw_label = |ui: &mut egui::Ui| {
            let tooltip = if let Some(waves) = &self.waves {
                if field.field.is_empty() {
                    self.signal_tooltip_text(waves, &field.root)
                } else {
